...
function showScreen(name, addHistory = true) {
  ...
  window.scrollTo(0, 0);
  const b = document.getElementById("btn-back"), p = document.getElementById("progress-bar");
  if (name === "type") { b.style.display = "none"; p.style.display = "none"; screenHistory = []; }
  else { b.style.display = "block"; p.style.display = "flex"; }
  const sm = { type:1, calendar:2, time:3, form:4, confirm:5, done:5 }, cur = sm[name] || 1;
  document.querySelectorAll(".step").forEach(s => {
    const n = parseInt(s.dataset.step);
    s.classList.remove("active","done");
    if (n === cur) s.classList.add("active");
    else if (n < cur) s.classList.add("done");
  });
}
...
function selectType(type) {
  state.type = type; state.date = null; state.time = null;
  ...
  renderCalendar(); showScreen("calendar");
}
...
async function fetchBookedSlots(date, type) {
  if (!CONFIG.gasUrl || CONFIG.gasUrl === "YOUR_GAS_URL_HERE") return [];
  try {
    const r = await fetch(CONFIG.gasUrl + "?date=" + date + "&type=" + type);
...
function renderTimeSlots(booked) {
  ...
  con.innerHTML = slots.map(s => {
    const b = isSlotBooked(s, booked, t.duration, t.capacity);
    return b ? '<button class="slot-btn booked" disabled>× ' + s + '</button>'
             : '<button class="slot-btn' + (state.time === s ? " selected" : "") + '" onclick="selectTime(\'' + s + '\')">' + s + '</button>';
  }).join("");
}
...
function isSlotBooked(slot, booked, dur, cap) {
  let c = 0;
  booked.forEach(b => {
    const bs = timeToMin(b.time), be = bs + (b.duration||1)*CONFIG.slotMinutes, ss = timeToMin(slot), se = ss + dur*CONFIG.slotMinutes;
    if (ss < be && se > bs) c++;
  });
  return c >= cap;
}
...
function selectTime(time) {
  state.time = time; renderTimeSlots([]);
  const t = CONFIG.types[state.type];
  document.getElementById("form-summary").textContent = t.emoji + " " + t.label + "　" + formatDateJp(state.date) + "　" + state.time + "〜" + addMinutes(state.time, t.duration*CONFIG.slotMinutes);
  showScreen("form");
}
...
function showConfirm() {
  ...
  document.getElementById("confirm-details").innerHTML =
    row("種別", t.emoji+" "+t.label) + row("日付", formatDateJp(state.date)) + row("時間", state.time+" 〜 "+end) +
    (t.showPeople ? row("人数", state.people+"名") : "") + row("お名前", state.name) + row("お電話", state.phone) +
...
async function submitBooking() {
  ...
  btn.disabled = true; btn.textContent = "送信中..."; showLoading(true);
  const t = CONFIG.types[state.type];
  ...
  let data = {};
  try {
    const res = await fetch(CONFIG.gasUrl + "?" + params.toString());
  ...
  if (ok) {
    const end = addMinutes(state.time, t.duration*CONFIG.slotMinutes);
    const lineHtml = data.lineUrl
      ? '<div style="margin-top:20px;text-align:center;padding:16px;background:#f0fff4;border-radius:12px;">'
        + '<p style="margin-bottom:12px;font-size:14px;color:#333;">当日のご案内をLINEでお送りします。<br>公式LINEの友だち追加をお願いします！</p>'
        + '<a href="' + data.lineUrl + '" target="_blank" style="display:inline-block;background:#06C755;color:#fff;padding:12px 28px;border-radius:8px;text-decoration:none;font-weight:bold;font-size:15px;">📲 LINE友だち追加はこちら</a>'
        + '</div>'
      : '';
    document.getElementById("done-details").innerHTML =
      row("種別", t.emoji+" "+t.label) + row("日付", formatDateJp(state.date)) + row("時間", state.time+" 〜 "+end) +
      (t.showPeople && state.people > 1 ? row("人数", state.people+"名") : "") +
      row("お名前", state.name) + row("お電話", state.phone) + lineHtml;
    showScreen("done");
  }
}
...
function resetApp() {
  ...
  screenHistory=[]; showScreen("type");
}

function showLoading(s) { document.getElementById("loading").style.display = s ? "flex" : "none"; }
let tt;
function showToast(m) { const t=document.getElementById("toast"); t.textContent=m; t.style.display="block"; if(tt)clearTimeout(tt); tt=setTimeout(()=>t.style.display="none",3000); }
